...
export function getLeaderboard(game?: string, limit = 10): LeaderboardEntry[] {
  ...
  if (game) {
    filteredLeaderboard = globalLeaderboard.filter((entry) => entry.game === game)
  }
...
export function addScore(
  ...
  console.log(`Adding score: ${playerName}, ${score}, ${game}, ${walletAddress || "no wallet"}`)
  ...
  if (existingEntryIndex !== -1) {
    ...
    if (shouldUpdate) {
      newEntry = {
        ...existingEntry,
        score: game === "minesweeper" ? score : Math.max(score, existingEntry.score),
  ...
  globalLeaderboard = globalLeaderboard.sort((a, b) => b.score - a.score).slice(0, 100)

  // Calculate rank for this game
  const gameLeaderboard = getLeaderboard(game, 50)
  const rank = gameLeaderboard.findIndex((entry) => entry.id === newEntry.id) + 1
  ...
  console.log(
    `New ${game} score: ${playerName} - ${score} ${game === "minesweeper" ? "wins" : "points"} (Rank #${rank})`,
  )
...
export function getPlayerRank(playerName: string, game: string): number {
  const gameLeaderboard = getLeaderboard(game, 50)
  const rank = gameLeaderboard.findIndex((entry) => entry.playerName === playerName) + 1
  return rank || -1 // Return -1 if not found
}

export function getPlayerScore(playerName: string, game: string): number {
  const entry = globalLeaderboard.find((entry) => entry.playerName === playerName && entry.game === game)
