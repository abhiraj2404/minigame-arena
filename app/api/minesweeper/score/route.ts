...
export async function POST(request: Request) {
  try {
    ...
    if (!playerName || typeof wins !== "number") {
      return NextResponse.json({ error: "Invalid player name or wins count" }, { status: 400 })
    }

    if (wins < 1) {
      return NextResponse.json({ error: "Wins must be at least 1" }, { status: 400 })
    }

    console.log(`Submitting Minesweeper win: ${playerName} - ${wins} - ${walletAddress}`)

    // Add win to shared leaderboard (score = number of wins)
    const result = addScore(playerName, wins, "minesweeper", walletAddress)
