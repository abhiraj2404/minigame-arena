...
const ADJECTIVES = ["Swift", "Master", "Pro", "Epic", "Super", "Mega", "Ultra", "Turbo", "Lightning", "Quantum"]
const NOUNS = ["Stacker", "Builder", "Clearer", "Wizard", "King", "Champion", "Legend", "Genius", "Hero", "Ace"]
...
export default function TetrisPage() {
  ...
  const loadLeaderboard = async (showLoading = false) => {
    try {
      ...
      if (response.ok) {
        ...
        console.log("Tetris leaderboard updated:", data.leaderboard?.length || 0, "entries")
      }
    } catch (error) {
  ...
  const initiateGame = () => {
    ...
    if (DEVELOPMENT_MODE) {
      ...
      startNewGame()
      setMessage("Development mode: Game started without payment")
      return
    }
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      <div className="text-center space-y-4">
        ...
        {message && (
          <div className="bg-green-900/50 border border-green-500/50 rounded-lg p-3 text-green-200">{message}</div>
        )}
      ...
      <div className="grid lg:grid-cols-4 gap-8">
            ...
            <div className="mb-6">
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 mb-4">
                <div className="px-3 py-2 bg-green-500/10 border border-green-500/30 rounded-lg">
                  <span className="text-green-400 font-semibold text-sm">Player: {gameState.playerName}</span>
                </div>
                ...
                {highScore > 0 && (
                  <div className="px-3 py-2 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                    <span className="text-yellow-400 font-semibold text-sm">High: {highScore.toLocaleString()}</span>
                  </div>
                )}
              </div>
              <div className="flex justify-center">
                ...
                ) : gameState.gameStatus === "waiting" || gameState.gameStatus === "gameOver" ? (
                  <ShimmerButton onClick={initiateGame} disabled={loading} className="text-sm">
                    {loading ? "Submitting..." : DEVELOPMENT_MODE ? "Start Game (Dev)" : "Start Game"}
                  </ShimmerButton>
                ) : (
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          {tournament && (
            <div className="bg-gray-900/50 backdrop-blur-sm border border-white/10 rounded-2xl p-6">
              <h3 className="text-xl font-bold text-white mb-4">🏆 Current Tournament</h3>
              <div className="space-y-3">
                <div className="flex justify-between">
                  <span className="text-gray-400">Entry Fee:</span>
                  <span className="text-green-400 font-bold">{tournament.entryFee} SOL</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">Prize Pool:</span>
                  <span className="text-blue-400 font-bold">{tournament.prizePool} SOL</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-yellow-400 font-bold">{formatTimeRemaining(tournament.endDate)}</span>
                </div>
              </div>
            </div>
          )}
                ...
                leaderboard.map((entry, index) => (
                  ...
                  >
                    ...
                    <div className="text-right">
                      <p className="text-green-400 font-bold">{entry.score.toLocaleString()}</p>
            ...
            {gameState.score > 0 && gameState.gameStatus === "gameOver" && (
              <div className="mt-4 p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg">
                <p className="text-blue-400 text-sm text-center">
                  Your final score: <span className="font-bold">{gameState.score.toLocaleString()}</span>
                  {gameState.score > highScore && (
                    <span className="block text-yellow-400 font-bold">🎉 New High Score!</span>
                  )}
                </p>
              </div>
            )}
